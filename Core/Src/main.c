...
void vHandlerTask( void *pvParameters )
{
	...
	{
		/* Take the semaphore */
		xSemaphoreTake(xSemaphore, 0xFFFF); // 有沒有 if == pdTRUE 都有相同的效果
		// semaphore was obtained
		if(xSemaphore != NULL)
		{
			// orange led blink five time
			for(int i = 0; i<10; i++)
			{
			...
			uint8_t outData;
			MEMS_Read(LIS3DSH_OUTS1_ADDR, &outData); // 自動 reset
		}
	}
}
...
int main(void)
{
  ...
  MEMS_Write(LIS3DSH_CTRL_REG1_ADDR, 0x01); //SM1_EN: 1
  MEMS_Write(LIS3DSH_CTRL_REG3_ADDR, 0x48); //IEA: 1 INT1_EN: 1
  MEMS_Write(LIS3DSH_CTRL_REG4_ADDR, 0x67); //ODR: 0110 Z Y Xen: 1
  MEMS_Write(LIS3DSH_CTRL_REG5_ADDR, 0x00);
  MEMS_Write(LIS3DSH_THRS1_1_ADDR, 0x55); //Threshold value
  MEMS_Write(LIS3DSH_MASK1_B_ADDR, 0xFC); //P_X N_X P_Y N_Y P_Z N_Z: 1
  MEMS_Write(LIS3DSH_MASK1_A_ADDR, 0xFC); //P_X N_X P_Y N_Y P_Z N_Z: 1
  MEMS_Write(LIS3DSH_ST1_1_ADDR, 0x05);
  MEMS_Write(LIS3DSH_ST1_2_ADDR, 0x11);
  MEMS_Write(LIS3DSH_SETT1_ADDR, 0x01); //SITR: 1
